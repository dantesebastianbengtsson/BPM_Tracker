...
export type AccentColor = 'violet' | 'blue' | 'teal' | 'green' | 'amber' | 'rose';
...
const DEFAULTS: Persisted = { theme: 'light', accent: 'violet', sound: 'click', volume: 0.6, beatsPerBar: 0 };

@Injectable({ providedIn: 'root' })
export class SettingsService {
  readonly theme = signal<ThemeMode>(DEFAULTS.theme);
  readonly accent = signal<AccentColor>(DEFAULTS.accent);
